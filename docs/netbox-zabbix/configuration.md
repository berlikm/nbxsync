...
Same template assignment on **MSSQL Query Server**. Leftover role `{$MSSQL.DSN}` is ODBC-era; ignore for Agent 2. Named instances (and their databases / Always On local DBs) are LLD on **MSSQL Observability** (import `zabbix/templates/mssql_observability/template_mssql_observability.yaml`, or `--apply-mssql`). Link that companion **next to** stock Agent 2 — do not nest stock, do not put instance names in NetBox, do not mute Test/Dev with `{$MSSQL.BACKUP_*.USED}=0` on the role.
