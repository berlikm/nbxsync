...
Operator procedures after the initial nbxSync build ([configuration checklist](../nbxsync-configuration-checklist-zerotouch.md) §§1–13 + plugin settings §12).
...
1. Does it need a **transport exception**?
   - Agent-class → nothing (Site Group Agent default)
...
3. New **Switch*** role? Copy IFALIAS / IFTYPE macros from the closest peer (`zabbix/01-extreme-switching.md` §5 / §8; nbxSync assignments per checklist §11.1). Platform Template Rules already cover EXOS/VOSS.
...
| Update “Last verified” stamp on the [configuration checklist](../nbxsync-configuration-checklist-zerotouch.md) | After a production re-validation |
