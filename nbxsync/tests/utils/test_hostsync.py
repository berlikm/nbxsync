...
class HostSyncTestCase(TestCase):
    ...
    def _assigned(self, templateid, requirements=None):
        class DummyZabbixTemplate:
            def __init__(self):
                self.templateid = templateid
                self.interface_requirements = requirements or [HostInterfaceRequirementChoices.NONE]

        class DummyAssignedTemplate:
            def __init__(self):
                self.zabbixtemplate = DummyZabbixTemplate()

        return DummyAssignedTemplate()

    def test_get_template_attributes_drops_nested_icmp_and_http(self):
        obs, http, icmp = 13901, 10002, 10001
        self.sync.context['all_objects']['templates'] = [
            self._assigned(icmp),
            self._assigned(obs),
            self._assigned(http),
        ]
        parents = {obs: [http, icmp], http: [icmp], icmp: []}

        def template_get(**kwargs):
            ids = [int(tid) for tid in kwargs.get('templateids') or []]
            return [
                {
                    'templateid': tid,
                    'parentTemplates': [{'templateid': pid} for pid in parents.get(tid, [])],
                }
                for tid in ids
            ]

        self.sync.api.template.get = template_get
        result = self.sync.get_template_attributes()
        self.assertEqual(result['templates'], [{'templateid': obs}])

    def test_get_template_attributes_keeps_snmp_sibling(self):
        obs, snmp = 13901, 13902
        self.sync.context['all_objects']['templates'] = [self._assigned(obs), self._assigned(snmp)]

        def template_get(**kwargs):
            ids = [int(tid) for tid in kwargs.get('templateids') or []]
            parents = {obs: [10002, 10001], snmp: []}
            return [
                {
                    'templateid': tid,
                    'parentTemplates': [{'templateid': pid} for pid in parents.get(tid, [])],
        ...
        self.sync.api.template.get = template_get
        result = self.sync.get_template_attributes()
        self.assertEqual(result['templates'], [{'templateid': obs}, {'templateid': snmp}])

    def test_get_template_attributes_skips_parent_fetch_for_one_template(self):
        self.sync.context['all_objects']['templates'] = [self._assigned(101)]

        def template_get(**kwargs):
            raise AssertionError('single template should not fetch parentTemplates')

        self.sync.api.template.get = template_get
        result = self.sync.get_template_attributes()
        self.assertEqual(result['templates'], [{'templateid': 101}])
