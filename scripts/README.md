...
| — | `test_sap_sensirion.py` | SAP HANA + SAP ME templates: openSUSE ST22 vs Windows jstart (no live sapcontrol / SNMP) |
...
export. The SH01 walk proved Linux Net-SNMP only. HANA OS is stock Linux
by SNMP (CPU cores / disk IO / FS / RAM / NICs; ping disabled). The SAP
YAML is ST22 / sapcontrol only. Certificate is the Zabbix agent
(`web.certificate.get`), not a proxy script. Application rows are local
sapcontrol. HANA is openSUSE (Python UserParameter if an agent exists).
ME is Windows (PowerShell + `jstart.exe`). `--apply-sap` cannot push those
files; see
...
That imports **SAP template from Sensirion** (HANA, ST22, AGENT req) and
**SAP ME from Sensirion** (Windows, AGENT req), assigns Linux by SNMP on
role SAP HANA (disables its ping), and HostSyncs only `CH-STA-P-SH01` when
that device exists and is not onboarding. It never imports the Extreme pack,
never runs zerotouch, and never fleet-syncs. Application triggers stay off
until `{$SAP.APP.CONTROL}=1` after the UserParameter works. Tests:
...
| SAP HANA + SAP ME from Sensirion (openSUSE ST22 + Linux by SNMP vs Windows agent) | soft-assign HANA / ME templates + Linux by SNMP on SAP HANA; Linux rule excludes SAP HANA | `--apply-sap` / `--check-sap` (HostSync only `CH-STA-P-SH01` if present and not onboarding; no Extreme import) |
