...
| IIS | Stock **IIS by Zabbix agent** covers W3SVC/WAS/port/app pools. HTTPS certs are companion **IIS Observability** (binding LLD, no FQDN list). Link both on the hosts; do not scrape windows_exporter — [notes/iis-coverage.md](notes/iis-coverage.md) |
