...
Stock `fgate.sdwan.get_data` / `fgate.netif.get_data` / `fgate.system.get_data` keep **history=0** (pass-through to dependents). `item.get` `lastclock=0` / empty `lastvalue` on those masters is **not** proof the script timed out — `fgate.ha.role` shows lastclock because apply created it with default history. ZH4/ZH5: `fgate.api.status` already has a current lastclock and is **dependent on** `fgate.system.get_data`, so that HTTP-agent master is running even while its own lastclock stays 0. Read `fgate.sdwan.data_errors`, `fgate.netif.data_errors`, LLD counts (`fgate.observability.sdwan.count` / `netif.count`), not the raw master lastclock. Apply sets those three masters to **1h** history so Execute now is visible. HostSync is not required for a script-item patch. These canaries use `proxyid=0` (server path); HA role already collects there — do not assume a Swiss proxy is required for this pair.
