...
VDOM: stock FortiGate by HTTP polls **the REST admin’s current VDOM only** (usually `root`) — there is no VDOM LLD. That is why ZH4 showed `fgate.device.vdom=root` and only `ha` / `mgmt` / `mgmt.loopback`. `--apply-fortigate-http` patches `fgate.netif.get_data` and `fgate.sdwan.get_data` to call `?vdom=*` (and `include_vlan=true` on the monitor iface API) and flatten the multi-VDOM array. `{#IFKEY}` becomes `vdom:ifName` so `port1` in two VDOMs does not collide. `{#IFNAME}` stays the Forti ifName so MATCHES still works. Non-200 (including **424** Failed Dependency) is empty for **that** endpoint, not a dead item: ZH5 `virtual-wan/health-check` is 424 on every VDOM while members/cmdb still 200 — collect members, skip health. **424/404 do not walk every VDOM** (that would blow `{$FGATE.DATA.TIMEOUT}`). 500 still retries per VDOM. Interface LLD needs `data` as an **array**; SD-WAN always emits `member_lld` / `health_lld`. The REST profile must have **read on every VDOM** you want named; `vdom=*` cannot see VDOMs the token cannot access. `fgate.device.vdom` remains the login VDOM, not a list. Do **not** put the token in the URL query (FortiOS 7.4.5+). Do **not** set `{$NET.IF.IFNAME.NOT_MATCHES}=.*` — LLD is MATCHES **and** NOT_MATCHES, so that hides every interface.

The live host bodies contain those functions, but Latest data still shows `ReferenceError: identifier 'fortiFetchVdom' undefined` / `flattenFortiMonitorMap` undefined and LLD counts 0 — Duktape does not see helpers **nested inside** `getHttpData`. The array-guard `return response` made the old inserter splice them there. Apply now lifts them to program scope. Until that apply, Latest data `Get interfaces data` / `Get SD-WAN data` stay blank (`history=0`) while `*.data_errors` holds the ReferenceError.

Stock raw masters also keep **history=0**. `item.get` lastclock on those items stays empty even when the poller is running — `fgate.ha.role` shows lastclock because apply created it with default history. Apply sets netif/sdwan/system masters to **1h** history so Execute now is visible. HostSync is not required for a script-item patch. ZH4/ZH5 use `proxyid=0`; HA role already collects there.
