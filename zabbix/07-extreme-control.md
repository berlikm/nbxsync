...
- Collection is Zabbix **SCRIPT** on the **Swiss proxy**, not `externalscripts`. Health every **2m**; NAC + Pilot census every **15m** (timeout 60s). If Latest data is an hour old, fix proxy pollers (`Timeout`, `StartSNMPPollers`) before debugging GraphQL — [notes/swiss-proxy-tuning.md](notes/swiss-proxy-tuning.md).
...
| Check | Why |
|---|---|
| GraphQL nodata | Token expired / SE upgrade / TLS. Also Swiss proxy **poller queue** — SCRIPT last-check hours late is not NBI down ([notes/swiss-proxy-tuning.md](notes/swiss-proxy-tuning.md)) |
...
| SCRIPT last-check far past interval | `xiqse.nbi.health` is 2m, licenses/pilot **15m**. An hour stale is `StartPollers` starvation on `ch-sta-p-zabp02` (`Timeout=30`, classic SNMP). Not an external script |
