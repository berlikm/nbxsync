...
|---|---|---|---|
| SAP ABAP / instance / IDoc / jobs / locks / qRFC in+out / RFC / spool / syslog / tRFC / updates | Promonitor / `C_PROMONITOR` (11 hosts). SH01 live row is `ABAPRuntimeErrorsCount_LMS` only | sapcontrol dependents on **SAP template from Sensirion** (Host Agent). `{$SAP.APP.CONTROL}=0` until the UserParameter is installed. Collection script not pasted yet | ⚠️ Template built; install UserParameter on the host |
