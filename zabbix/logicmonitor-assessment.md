...
| **WMI** (DC override `CORP-UPA-Dom_Monitor`) | Domain Controllers (AD + DNS + DHCP on the same VM) | Windows by agent = OS + Automatic services. NTDS / DNS / DHCP-scope counters are **not** in stock 7.0 — [notes/ad-ds-coverage.md](notes/ad-ds-coverage.md) | ⚠️ OS covered; DC roles gap |
...
| IIS | couple of Windows web servers | ⚠️ stock template exists | link on Device | **IIS by Zabbix agent** 7.0 for W3SVC/pools; **IIS Observability** for HTTPS binding certs (no FQDN list) — [notes/iis-coverage.md](notes/iis-coverage.md); not zerotouched |
