...
└── extreme_port_mute.py        # allowlist shutdown / no-shutdown / X-
```

Port shutdown / no-shutdown / `X-` mute is a **separate** script so cabling
remediate cannot disable (or enable) ports:
[`README-extreme-port-mute.md`](README-extreme-port-mute.md).
