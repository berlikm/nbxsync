...
- Honeycomb thresholds are `>=`. VOSS fan `notpresent(4)` paints like `down(3)` (red). PSU LLD keeps a row when status is not empty/notPresent **or** serial is set; padding (empty, no serial) is skipped. EXOS stack padding slots often have **no** `extremePowerSupplySerialNumber` instance, so Zabbix omits `{#PSU.SERIAL}` and the OR filter errors (`no value received for macro`) until LLD JS defaults the missing macros to empty. `--apply` writes that OR filter without `formulaid` (Zabbix 7 rejects IDs on AND/OR; GET still canonicalizes to `A or B`). YAML export may still list A/B. VOSS **deletes** lost padding immediately — honeycomb keeps lastvalue on a disabled item. Two present / one connected tickets Average: `last()<>{$PSU.OK_STATUS}` (VOSS `unknown(1)`, serialled `empty(2)`, `down(4)`; EXOS `notPresent(1)` with serial, `presentNotOK(3)`, `presentPowerOff(4)`). VOSS honeycomb already paints `empty(2)` red (`>=1` until `up(3)`). EXOS honeycomb adds threshold `1` red so serialled `notPresent` is red, `presentOK(2)` green, `>=3` red. `--apply` queues check-now PSU LLD on those hosts; it does not host-sync or write NetBox.
