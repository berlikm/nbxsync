...
- Honeycomb thresholds are `>=`. VOSS fan `notpresent(4)` paints like `down(3)` (red). VOSS PSU LLD keeps a row when status is **not** `empty(2)`. Dummy serial `--` is not a FRU (CH-STA-L26-L02-MGMT03: one PSU, PSU 2 empty/`--`). EXOS keeps status not `notPresent` **or** a real serial. EXOS stack padding slots often have **no** `extremePowerSupplySerialNumber` instance, so Zabbix omits `{#PSU.SERIAL}` and the filter errors (`no value received for macro`) until LLD JS defaults missing macros and wipes dummies. `--apply` writes that filter without `formulaid` (Zabbix 7 rejects IDs on AND/OR; GET still canonicalizes to `A or B` / `A`). YAML export may still list A/B. VOSS **deletes** lost padding immediately — honeycomb keeps lastvalue on a disabled item. Two present / one connected tickets Average: `last()<>{$PSU.OK_STATUS}` (VOSS also `and last()<>{$PSU.EMPTY_STATUS}`). VOSS honeycomb paints `empty(2)` red (`>=1` until `up(3)`), so skipping LLD is what removes the second hex. EXOS honeycomb adds threshold `1` red so serialled `notPresent` is red, `presentOK(2)` green, `>=3` red. `--apply` queues check-now PSU LLD on those hosts; it does not host-sync or write NetBox.
- Unused chassis OOB is not a labelled data port. VOSS ifName `mgmt` and EXOS ifName `Management` (`MgmtPort`) are skipped in `{$NET.IF.IFNAME.NOT_MATCHES}`. Mute data ports with `X`, not `{$IFCONTROL}`. `--apply` queues check-now IF LLD for hosts still holding those rows.
