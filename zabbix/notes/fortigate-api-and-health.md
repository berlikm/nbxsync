...
| No `vdom=*` on iface/SD-WAN scripts | Only the REST login VDOM (usually `root`) is discovered — other VDOMs look empty | Apply patches `fgate.netif.get_data` / `fgate.sdwan.get_data`. `vdom=*` **500** retries per-VDOM then login VDOM. **424/404** (ZH5 `virtual-wan/health-check`) return empty for **that** endpoint and must not walk VDOMs. Members/cmdb stay independent. LLD always has `member_lld` so JSONPath stays supported |
| Raw masters `history=0` | `item.get` lastclock/lastvalue stay empty on `fgate.*.get_data`; looks like “never collected” while dependents (`fgate.api.status`, `*.data_errors`, LLD) update | Apply sets netif/sdwan/system masters to **1h** history. Debug those dependents until then. `fgate.ha.role` was never history=0 |
