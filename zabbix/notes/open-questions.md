...
- [ ] **IIS Observability** — YAML built (`templates/iis_observability/`). Linked on `CH-STA-P-WEBN02` (canary 2026-08-30): `*:443:` handshake + expiry work; empty host header must show `not_evaluated_no_host_header`, not Agent 2 loopback `invalid`. Re-import after that fix. Still: link the other IIS VM; Agent 2.
