...
| Official website cert template in 7.0 | **Website certificate by Zabbix agent 2** — `web.certificate.get`; Warning at `{$CERT.EXPIRY.WARN}` default 7d; one hostname per host (no IIS binding LLD). **Not** the IIS-site path — we have no FQDN list | git.zabbix.com `templates/app/certificate_agent2` release/7.0 |
| IIS HTTPS binding certs | Companion **IIS Observability** — LLD `https` bindings from `applicationHost.config`, handshake on the box, Warning 30d. Needs Agent 2. Does not nest stock IIS | [templates/iis_observability/](../templates/iis_observability/); [iis-coverage.md](iis-coverage.md) |
