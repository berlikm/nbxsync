...
| Chassis with two PSU bays | SNMP has a row per bay (`empty(2)` on VOSS, `notPresent(1)` on EXOS) | Padding is skipped. VOSS `empty(2)` is not installed even when firmware fills serial with `--` (CH-STA-L26-L02-MGMT03). EXOS padding indexes often have **no** serial OID — LLD JS defaults `{#PSU.SERIAL}` and wipes dummy values so the filter can run. VOSS **deletes** lost padding immediately. Two present / one connected Average (`last()<>{$PSU.OK_STATUS}`; VOSS also excludes empty). Fitted VOSS unplugged is `unknown` / `down`; EXOS is `presentPowerOff` / serialled `notPresent` |
...
| **2** | Link down, flapping, PSU, fan, temperature **critical** | Padding PSU bays skipped (empty/notPresent with no serial); two present / one connected Average; stack-temperature too-low silenced; < 5 alerts/day |
...
- [x] ~~PSU status reported by an **empty** slot in a 2-PSU chassis~~ **Answered** — SNMP keeps a row per bay. VOSS `empty(2)` is not installed. Chassis firmware often fills serial with `--` on every bay (canary CH-STA-L26-L02-MGMT03: PSU 1 `up(3)` / 90 W, PSU 2 `empty(2)` / 0 W / `--`). Dummy serial is not a FRU; LLD skips empty even when serial looks set, and Average excludes `{$PSU.EMPTY_STATUS}` so leftover PSU 2 recovers before LLD delete. Honeycomb paints `empty(2)` red (`>=1` until `up(3)`), so skipping LLD is what removes the second hex. EXOS padding is `notPresent` with no serial OID — LLD JS defaults `{#PSU.SERIAL}` and wipes dummies; a serialled unplugged EXOS FRU still tickets. VOSS **deletes** lost padding immediately (`lifetime: 0`).
