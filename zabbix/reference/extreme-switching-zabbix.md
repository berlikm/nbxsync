...
| Chassis with two PSU bays | SNMP has a row per bay (`empty(2)` on VOSS, `notPresent(1)` on EXOS) | Padding (empty/notPresent **and** no serial) is skipped. EXOS padding indexes often have **no** serial OID — LLD JS defaults `{#PSU.SERIAL}` so the filter can run. VOSS **deletes** lost padding immediately. Two present / one connected Average (`last()<>{$PSU.OK_STATUS}`), including serialled empty/notPresent, `unknown` / `presentPowerOff`, `down` / `presentNotOK` |
...
- [x] ~~PSU status reported by an **empty** slot in a 2-PSU chassis~~ **Answered** — SNMP keeps a row per bay. Padding is `empty(2)` / `notPresent(1)` **with no serial** (canary CH-STA-L26-L02-MGMT03 listed only PS#1 UP). EXOS padding slots often omit `extremePowerSupplySerialNumber`, so `{#PSU.SERIAL}` is missing until LLD JS defaults it; otherwise the OR filter is not-supported. LLD skips padding and VOSS **deletes** lost padding immediately (`lifetime: 0`; honeycomb keeps lastvalue on a disabled item). Two present / one connected Average: `last()<>{$PSU.OK_STATUS}` on discovered FRUs, including firmware that reports a fitted unplugged unit as empty/notPresent when serial is set. EXOS honeycomb paints serialled `notPresent(1)` red.
