# Extreme VOSS by SNMP ΓÇö OID mapping vs Extreme EXOS by SNMP
...
| PSU status LLD | `1916.1.1.1.9.1.2` | `rcChasPowerSupplyOperStatus` | `2272.1.4.8.1.1.2.{id}` | 1=unk 2=empty 3=up 4=down; **crit=4**; LLD skips **empty(2)**; lost empty rows **delete immediately** |
...
| `{$PSU.EMPTY_STATUS}` | `2` | `empty(2)` not installed (LLD skips even when serial is `--`) |
...
| Check | Result |
|---|---|
| `rcKhiSlotCpuCurrentUtil.1` | PASS (values ~40ΓÇô80%) |
