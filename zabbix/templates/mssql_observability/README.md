...
LLD-discovers `MSSQL$*` Windows services via WMI (`LIKE 'MSSQL%'`, JS keeps
`MSSQL$…`) and calls the Agent 2 plugin
...
- WMI master `Win32_Service WHERE Name LIKE 'MSSQL%'` (JS keeps `MSSQL$…`; not `service.discovery`)
